    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git credentialsId: 'jenkins_test', url: 'https://github.com/kalmamatova/jenkins-test.git'
            }
        }
...
def updateGitHubPRStatus(state, description) {
    ...
    def githubToken = credentials('github-token-alma') // Use Jenkins credentials to retrieve the GitHub token
